...
LICENSE="LGPL-2.1"
SLOT="0"
IUSE="alsa capi cups custom-cflags dbus +dns_sd esd fontconfig +gecko gnutls gphoto2 gsm gstreamer hal jack jpeg lcms ldap mp3 nas ncurses nls openal +opengl +oss +perl png pulseaudio samba scanner ssl test +threads +truetype v4l +win32 +win64 +X xcomposite xinerama xml"
RESTRICT="test" #72375
...
DEPEND="${RDEPEND}
	...
	sys-devel/bison
	sys-devel/flex
	dns_sd? ( net-dns/avahi[mdnsresponder-compat] )"
...
src_prepare() {
	...
	EPATCH_OPTS=-p1 epatch "${FILESDIR}"/dnspatch.patch # add dns support
	autoconf
	autoheader
}
